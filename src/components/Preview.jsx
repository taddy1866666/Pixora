...
export default function Preview({ onTip, onRestart }) {
  ...
  return (
    <div className="min-h-dvh bg-pixora-bg text-pixora-fg overflow-x-hidden flex flex-col items-center justify-center p-4 sm:p-6">
      <div className="flex items-center justify-center gap-0 mb-6 sm:mb-8">
        {['Layout', 'Capture', 'Edit', 'Preview'].map((item, idx) => (
          <div
            key={item}
            className={`px-4 sm:px-6 py-2 text-[10px] sm:text-xs font-bold tracking-[0.2em] uppercase border transition-all duration-300 ${idx === 3 ? 'bg-white text-black border-white' : 'bg-black text-white border-white/20'
              }`}
          >
            {item}
          </div>
      ...
      <canvas
        ref={canvasRef}
        className="shadow-2xl shadow-black/40 bg-white animate-scale-in mb-4"
        style={{ maxWidth: canvasMaxWidth, width: '100%', height: 'auto' }}
      />

      {/* Social Links */}
      <div className="mb-2 text-center">
        <p className="type-caption text-pixora-muted font-bold mb-1">Follow us</p>
        <div className="flex gap-2 justify-center">
          <a href="https://www.facebook.com/ksosbdiv/" target="_blank" rel="noopener noreferrer" className="text-white/80 hover:text-white transition-colors">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
              <path d="M24 12.073c0-6.627-5.373-12-12-12s-12 5.373-12 12c0 5.99 4.388 10.954 10.125 11.854v-8.385H7.078v-3.47h3.047V9.43c0-3.007 1.792-4.669 4.533-4.669 1.312 0 2.686.235 2.686.235v2.953H15.83c-1.491 0-1.956.925-1.956 1.874v2.25h3.328l-.532 3.47h-2.796v8.385C19.612 23.027 24 18.062 24 12.073z" />
            </svg>
          </a>
          <a href="https://www.instagram.com/ksosbdiv/" target="_blank" rel="noopener noreferrer" className="text-white/80 hover:text-white transition-colors">
            <svg className="w-4 h-4" fill="currentColor" viewBox="0 0 24 24">
      ...
      <div className="flex flex-wrap justify-center gap-2 animate-slide-up">
        <button
          onClick={onRestart}
          className="btn-ghost btn-pill group px-3 sm:px-4 py-2 font-semibold text-xs"
        >
        ...
        <button
          ...
          className="btn-primary btn-pill group px-4 sm:px-5 py-2 relative overflow-hidden text-xs"
        >
